...
namespace Tests\Unit\App\Http\Controllers\Api\V1\ArticleApiController;

use App\Contracts\ArticleApiControllerContract as Contract;
...
class ArticleApiControllerTest extends TestCase
{
    ...
    public function test_should_implement_ArticleApiControllerContract():void {
        $parent = get_parent_class($this->namespace);
        $parentImplements = class_implements($parent);
        $selfImplements = class_implements($this->namespace);
        $diff = array_diff($selfImplements,$parentImplements);
