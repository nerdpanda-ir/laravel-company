...
namespace Tests\Unit\NamespaceableEntityTestCaseTrait\Methods;
...
class MethodsTest extends TestCase
{
    ...
    public function test_should_have_double_method_from_use_traits():void {
        ...
        $traitReflection = new ReflectionClass($uses[array_key_first($uses)]);
        $methods = $traitReflection->getMethods();
        $expect = 2 ;
        $this->assertCount($expect,$methods,"$this->namespace trait should have two method from used traits !!! ");
    }
    public function test_should_have_one_method_from_HasNamespaceGetterTrait():void {
        ...
        foreach ($methods as $method)
            if ($method->getFileName()==$traitReflection->getFileName())
                $counter++;
        $this->assertEquals(1,$counter,"$this->namespace trait should just have one method from trait ".HasNamespaceGetterTrait::class);
    }
    public function test_methods_from_HasNamespaceGetterTrait_should_is_public(){
        ...
        if (!empty($traitMethods)){
            ...
            $this->fail("no found any methods in use trait ".HasNamespaceGetterTrait::class." in trait $this->namespace");
    }
    public function test_methods_from_HasNamespaceGetter_should_is_no_static():void {
        ...
        if (!empty($traitMethods)){
            ...
            if ($staticCount>0){
                ...
                $message.="in used trait ".HasNamespaceGetterTrait::class." in $this->namespace trait dont be static ";
            }
    ...
    public function test_methods_in_HasNamespaceGetterTrait_should_no_final():void {
        ...
        if (!empty($traitMethods)){
            ...
            if (!empty($finalMethods)){
                ...
                $message .="in used trait".HasNamespaceGetterTrait::class . " in $this->namespace trait dont be final !!!";
            }
            $this->assertEmpty($finalMethods,$message);
        }
        else
            $this->fail("no found any method in use trait ".HasNamespaceGetterTrait::class . " in trait $this->namespace ");
    }
    public function test_methods_in_HasNamespaceGetterTrait_should_no_abstract():void {
        ...
        if (!empty($traitMethods)){
            ...
            if (!empty($abstractMethods)){
                ...
                $message .= "in used trait".HasNamespaceGetterTrait::class . " in $this->namespace trait dont be abstract !!!";
            }
            $this->assertEmpty($abstractMethods,$message);
        }
        else
            $this->fail(" no found any method in used trait ".HasNamespaceGetterTrait::class ." in $this->namespace trait !!!");
    }
    public function test_should_have_one_method_from_HasNamespaceSetterTrait():void {
        ...
        foreach ($methods as $method)
            if ($method->getFileName()==$traitFile)
                $counter++;
        $this->assertEquals(1,$counter,"$this->namespace trait should just have one method from trait ".HasNamespaceSetterTrait::class);
    }
    public function test_methods_from_HasNamespaceSetterTrait_should_is_public(){
        ...
        if (!empty($traitMethods)){
            ...
            $this->fail("no found any methods in use trait ".HasNamespaceSetterTrait::class." in trait $this->namespace");
    }
        ...
        if (!empty($traitMethods)){
            ...
            if ($staticCount>0){
                if ($staticCount==1)
                    ...
                    $message ="methods ".implode(', ',$staticMethods);
                $message.="in used trait ".HasNamespaceSetterTrait::class." in $this->namespace trait dont be static ";
            }
            $this->assertEmpty($staticMethods,$message);
        }
        else
        ...
        if (!empty($traitMethods)){
            ...
            if (!empty($finalMethods)){
                ...
                if ($finalMethodsCount==1)
                    ...
                    $message = "methods ".implode(' , ',$finalMethods).' ';
                $message .="in used trait".HasNamespaceSetterTrait::class . " in $this->namespace trait dont be final !!!";
            }
            $this->assertEmpty($finalMethods,$message);
        }
        else
            $this->fail("no found any method in use trait ".HasNamespaceSetterTrait::class . " in trait $this->namespace ");
    }
        ...
        if (!empty($traitMethods)){
            ...
            if (!empty($abstractMethods)){
                ...
                if ($abstractMethodsCount==1)
                    ...
                    $message = "methods ".implode(' , ',$abstractMethods) ;
                $message .= "in used trait".HasNamespaceSetterTrait::class . " in $this->namespace trait dont be abstract !!!";
            }
            $this->assertEmpty($abstractMethods,$message);
        }
        else
            $this->fail(" no found any method in used trait ".HasNamespaceSetterTrait::class ." in $this->namespace trait !!!");
    }
}
